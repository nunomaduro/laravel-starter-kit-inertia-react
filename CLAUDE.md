...
- `developing-with-prism` — Activate ONLY when working with MCP tool integration via `PrismService::withTools()` (Relay bridge) or `PrismService::isAvailable()`. For all other AI features — text generation, structured output, images, audio, embeddings, streaming, agents — use `developing-with-ai-sdk` instead.
...
- In this project, Prism's role is intentionally narrow. Use `laravel/ai` Agent classes for all AI work.
- The only exception is `PrismService::withTools()`, which bridges Relay MCP tools into a Prism request — Relay has no `laravel/ai` equivalent.
- IMPORTANT: Activate `developing-with-prism` skill ONLY when working with MCP/Relay tool integration via `PrismService::withTools()`. For all other AI features, activate `developing-with-ai-sdk`.
