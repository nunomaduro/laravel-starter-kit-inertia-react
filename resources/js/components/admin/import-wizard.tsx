...
function ImportWizard({
  ...
  const handleMappingChange = (csvColumn: string, fieldId: string) => {
    setMapping((prev) => ({ ...prev, [csvColumn]: fieldId === "skip" ? "" : fieldId }))
  }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      clearInterval(ticker)
      setImportProgress(100)
      setImportResult({ success: 0, errors: 1, total: 1, errorDetails: ["Import failed. Please try again."] })
    } finally {
      setImporting(false)
    }
  }
  ...
  const mappedHeaders =
    parsed?.headers.filter((h) => mapping[h]) ?? []
      ...
      {step === "mapping" && parsed && (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Match your CSV columns to the target fields. Unmatched columns will
            be skipped.
          </p>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {Object.keys(validationErrors).length > 0 && (
              <span className="text-destructive font-medium">
                {Object.keys(validationErrors).length} row(s) have validation
                errors.
              </span>
            )}
          </p>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {mappedHeaders.map((h) => (
                    <TableHead key={h}>
                      {targetFields.find((f) => f.id === mapping[h])?.label ??
                        h}
                    </TableHead>
                  ))}
              ...
              <TableBody>
                {previewRows.map((row, rowIndex) => {
                  const hasError = Boolean(validationErrors[rowIndex])
                  return (
                    <TableRow
                      key={rowIndex}
                      ...
                      <TableCell>
                        {hasError && (
                          <AlertCircleIcon
                            ...
                            aria-label={validationErrors[rowIndex]?.join(", ")}
                          />
                        )}
                      </TableCell>
                    </TableRow>
