...
function ImpersonationBanner({
  ...
  const impersonatedUser = authAny.impersonated_user as { name?: string } | undefined

  if (!impersonating) return null

  const userName = impersonatedUser?.name ?? auth.user?.name ?? "another user"
  ...
  return (
    ...
    >
      ...
      <a
        href={leaveUrl}
        className="rounded border border-amber-700 px-2 py-0.5 text-xs font-semibold hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-700"
      >
        Stop Impersonating
      </a>
    </div>
