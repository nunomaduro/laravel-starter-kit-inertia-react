  ...
  return (
    ...
    >
      ...
      <span>
        {maintenance.message}
        {formattedDate && (
          <span className="ml-1 opacity-75">— scheduled {formattedDate}</span>
        )}
      </span>
      <button
        onClick={() => setDismissed(true)}
        aria-label="Dismiss maintenance notice"
        className="absolute right-3 top-1/2 -translate-y-1/2 rounded p-1 opacity-70 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-yellow-700"
      >
...
export type { MaintenanceInfo, MaintenanceBannerProps }
